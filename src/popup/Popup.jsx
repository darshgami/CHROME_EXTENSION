...
export default function App() {
  ...
  const [matchExact, setMatchExact] = useState(true);
  const [matchFuzzy, setMatchFuzzy] = useState(true);
  const [matchSynonyms, setMatchSynonyms] = useState(true);
  const [minRelevance, setMinRelevance] = useState(50);

  // Filter State
  const [filterHighRel, setFilterHighRel] = useState(false);
    ...
    chrome.storage.local.get(['lsp_leads', 'lsp_current_session'], (result) => {
      ...
      if (result.lsp_current_session) {
        ...
        if (result.lsp_current_session.config) {
          ...
          if (result.lsp_current_session.config.matchExact !== undefined) setMatchExact(result.lsp_current_session.config.matchExact);
          if (result.lsp_current_session.config.matchFuzzy !== undefined) setMatchFuzzy(result.lsp_current_session.config.matchFuzzy);
          if (result.lsp_current_session.config.matchSynonyms !== undefined) setMatchSynonyms(result.lsp_current_session.config.matchSynonyms);
          if (result.lsp_current_session.config.minRelevance !== undefined) setMinRelevance(result.lsp_current_session.config.minRelevance);
        }
      }
    });
      ...
      <main className="flex-1 overflow-y-auto py-3 space-y-4 pr-1">
        ...
        <section className="bg-slate-900/60 border border-slate-800/80 p-3 rounded-xl shadow-lg space-y-3">
          ...
          <div className="pt-2 border-t border-slate-800/80">
            <label className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 block mb-2">Match Types</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                <input type="checkbox" checked={matchExact} onChange={(e) => setMatchExact(e.target.checked)} disabled={isRunning} className="accent-indigo-500" /> Exact
              </label>
              <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                <input type="checkbox" checked={matchFuzzy} onChange={(e) => setMatchFuzzy(e.target.checked)} disabled={isRunning} className="accent-indigo-500" /> Fuzzy
              </label>
              <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                <input type="checkbox" checked={matchSynonyms} onChange={(e) => setMatchSynonyms(e.target.checked)} disabled={isRunning} className="accent-indigo-500" /> Synonyms
              </label>
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Min Relevance</label>
              <span className="text-xs font-bold text-indigo-400">{minRelevance}%</span>
            </div>
            <input type="range" min="0" max="100" step="5" value={minRelevance} onChange={(e) => setMinRelevance(Number(e.target.value))} disabled={isRunning} className="w-full accent-indigo-500" />
          </div>
        </section>
        ...
        <section className="min-h-[140px] max-h-[180px] overflow-hidden">
          ...
          <div className="h-full flex flex-col">
            <div className="flex justify-between items-end mb-2">
              <div>
                <span className="text-[10px] text-slate-400 font-semibold uppercase block mb-1">Lead Filters</span>
                <div className="flex gap-2">
                  <label className="flex items-center gap-1 text-[9px] text-slate-400 cursor-pointer">
                    <input type="checkbox" checked={filterHighRel} onChange={(e)=>setFilterHighRel(e.target.checked)} className="accent-indigo-500" /> &ge; 70% Score
                  </label>
                  <label className="flex items-center gap-1 text-[9px] text-slate-400 cursor-pointer">
                    <input type="checkbox" checked={filterHasPhone} onChange={(e)=>setFilterHasPhone(e.target.checked)} className="accent-indigo-500" /> Has Phone
                  </label>
                  <label className="flex items-center gap-1 text-[9px] text-slate-400 cursor-pointer">
                    <input type="checkbox" checked={filterHasEmail} onChange={(e)=>setFilterHasEmail(e.target.checked)} className="accent-indigo-500" /> Has Email
                  </label>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto border border-slate-800 rounded-lg bg-slate-950">
                <table className="w-full text-left text-[11px] border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-900">
                    {leads
                      .filter(l => !filterHighRel || l.relevanceScore >= 70)
                      .filter(l => !filterHasPhone || l.phone || l.whatsapp)
                      .filter(l => !filterHasEmail || l.email)
                      .slice(-20).reverse().map((l, idx) => (
