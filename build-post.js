...
const extensionLibsDir = path.join(__dirname, 'extension', 'libs');
...
for (const option of fuseOptions) {
  if (fs.existsSync(option)) {
    ...
    console.log(`✓ Copied fuse.js from ${path.basename(option)} to extension/libs/fuse.js`);
    copiedFuse = true;
    break;
  }
}
...
if (fs.existsSync(papaSrc)) {
  ...
  console.log('✓ Copied papaparse.js to extension/libs/papaparse.js');
} else {
